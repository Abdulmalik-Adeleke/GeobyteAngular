...
export class NavigateroutesComponent implements OnInit {
    ...
    constructor(private shareddata: ShareddataService, private router: Router, private staff: StaffService) { }

    async ngOnInit(): Promise<void> {
        this.startcity = await this.staff.getStartHubs();
    ...
    async originchanged(start: string) {
        this.destinations = await this.staff.getDestinationHubs(start);
        this.origin = start;
    }
    ...
    async fetchroutes(){
        ...
        this.results = await this.staff.getRoutes(this.origin,this.destination,this.radius);
    ...
    navigatethrough() {
        ...
        console.log('this is shared data. get routeid and loop through all hubs and add orders to array');
        let data = this.pickedroute.data;
        data.forEach(Hub => {
            Hub.deliveryAddresses.forEach(order => {
                ignoreids.push(order.id);
            });
        });
       this.router.navigate(['/staff/more-routes'],{ queryParams: { routeId: this.pickedroute.routeId,idsToIgnore: ignoreids} });
    }
